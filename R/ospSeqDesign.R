...
osp.seq.design <- function(model,method="km")
{
  ...
  theta.fit <- array(0, dim=c(M,model$adaptive.grid.loop-model$init.size+1,model$dim))
  ...
  for (i in (M-1):1)
  {
    ...
    if (model$dim > 1) {
      ...
      K0 <- length(init.grid)
      big.grid <- as.matrix(init.grid[ rep(1:K0, model$km.batch)])
    }
    ...
    while (add.more.sites == TRUE)
    {
      ...
      if (model$al.heuristic == 'sur')  
        al.weights <- cf.sur(cand.mean, cand.sd, nugget = nug)
      if (model$al.heuristic == 'tmse')
