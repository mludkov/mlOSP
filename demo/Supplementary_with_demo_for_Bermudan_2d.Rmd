...
The code is part of a larger R library for Bermudan option pricing which can be provided upon request (hosted on GitHub, link hidden to maintain author identity for double-blind review process).
...
model2d$max.lengthscale <- c(40,40)  # bounds on lengthscales for the squared-exp kernel
...
oos.obj.adsa <- osp.seq.batch.design.simplified(model2d)

### GP + ABSUR
set.seed(122)
model2d$batch.heuristic <- 'absur'
model2d$ei.func <- 'absur'
oos.obj.absur <- osp.seq.batch.design.simplified(model2d)
```
...
The function's main input is \textit{model} which is the list defining the Bermudan option and GP parameters.  
...
```{r}
osp.seq.batch.design.simplified <- function(model, method="trainkm")
{
  M <- model$T/model$dt
